 ...
 */
class L_StatusStanowiskaUpdate extends \crmsw\lib\a\BusinessLogic{
	...
	protected function logic() {
		foreach ($this->dataIn as $key => $row) {
			try{
				$Record = $this->StatusyStanowiskTable->getRecord($row['id']);
				$StatusStanowiska = $Record->getEncja();
				$StatusStanowiska->setSymbol($row['symbol']);
				$StatusStanowiska->setOpis($row['opis']);
