 ...
 */
class L_BankiCreate extends \crmsw\lib\a\BusinessLogic{
  ...
  protected function logic() {
    foreach ($this->dataIn as $key => $row) {
      try{
        $Bank = new Bank($row['symbol'], $row['nazwa'], $row['nip'], $row['kod_poczt'], $row['miejscowosc'], $row['ul'], $row['nr_b'], $row['nr_l'], $row['tel'], $row['email'], $row['data_od'],null);
        $this->dataOut[$key] = array('id'=>$this->BankiTable->createRecordImmediately($Bank)->getId(),'tmpId'=>  $row['tmpId']);
      }  catch (Exception $E){
				$this->success = FALSE;
