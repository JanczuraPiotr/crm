 ...
 */
class L_PrezesCreate extends \crmsw\lib\a\BusinessLogic{
  ...
  protected function logic() {
    foreach ($this->dataIn as $key => $row) {
      try{
				...
				// Zmieniam prezesa więc bez zastanowienia usówam poprzedniego prezesa
				// Jeżeli poprzedni prezes był zlinkowany ze stanowiskiem pracy
				...
				if($row['status_stanowiska_id'] === 1 ){ // 1 jest identyfikatorem prezesa zarządu
					...
					if($eStanowisko->getStatusStanowiskaId() === $row['status_stanowiska_id']){
						...
						if(!$kolejny_prezes){
							...
							throw $e = new \pjpl\depreciate\EDBNotUnique(
											__CLASS__,
											__FUNCTION__,
	...
	private function create($key,$row){
		$Stanowisko = new Stanowisko(
						...
						$row['nazwa'],
						$row['placowka_id'],
						null, // $row['pracownik_id'], Przypisywanie pracownika możliwa tylko przez logikę zwalaniania i zatrudniania na stanowisku
						$row['tel'],
						$row['email'],
						$row['status_stanowiska_id'],
						$row['data_od'],
